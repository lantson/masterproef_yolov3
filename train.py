...
flags.DEFINE_enum('transfer', 'none',
                  ['none', 'darknet', 'no_output', 'frozen', 'fine_tune', 'continue'],
...
def setup_model():
    if FLAGS.tiny:
        model = YoloV3Tiny(training=True, channels=3,
...
def main(_argv):
    ...
    tf.config.experimental.set_memory_growth(physical_devices[0], True) # zelf nog toegevoegd
    ...
    if FLAGS.mode == 'eager_tf':
        ...
        callbacks = [
            ...
            ModelCheckpoint('checkpoints/yolov3_tiny_3_band_{epoch}.tf', verbose=1, save_best_only=True, save_weights_only=True),
