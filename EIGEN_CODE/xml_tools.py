...
class PascalXml(object):
  ...
  def replace_filename(self, new_filename):
    """Replace an object name.
    Args:
      old_name: str, an object class name.
      new_name: str, a new object class name.
    ...
    for image_bbox in split_bboxes:
      ...
      for bbox in self.object:
        ...
        if iou([ob_xmin, ob_ymin, ob_xmax, ob_ymax],
          ...
          sub_bbox = Bndbox(input_xmin - img_xmin, input_ymin - img_ymin,
                            input_xmax - img_xmin, input_ymax - img_ymin)
...
def load_pascal_xml(xml_file_path: str, default_format=None) -> PascalXml:
  ...
  for obj in annotation[0].xpath('./object'):
...
def save_pascal_xml_with_new_filename(save_xml_path: str, pascal_xml: PascalXml, new_filename) -> PascalXml:
  """save data to xml
  ...
  set_sub_node_info(node_root, 'filename', new_filename)
  ...
  for obj in pascal_xml.object:
    ...
    set_sub_node_info(node_object, 'pose', "Unspecified")
...
def save_pascal_xml_with_new_filename_and_horizontal_flip(save_xml_path: str, pascal_xml: PascalXml, new_filename, size) -> PascalXml:
  """save data to xml
...
class XmlParser():
  ...
  def __init__(self):
    raise DeprecationWarning('Please use new class PascalXml')
